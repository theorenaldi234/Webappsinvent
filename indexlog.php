...
<div class="col-lg-3 mb-3">
    <div class="card small mb-3">
        <div class="card-header p-2">
            <div class="card-tittle"><i class="far fa-file mr-1"></i>In Logistik</div>
        </div>
...
if(isset($_POST['tambahcuy'])){
    ...
    $sisa      = $stock+$quantity;
     ...
     $insert = mysqli_query($conn, "INSERT INTO keranjanglog (idproduk,quantity) VALUES ('$idproduk','$quantity')");
      if($insert){
        $upstok = mysqli_query($conn, "UPDATE produklog SET stock='$sisa' WHERE idproduk='$idproduk'");
        echo '<script>window.location="indexlog.php"</script>';
     }
...
<div class="d-none pt-5 px-4 print-show">
    <div class="row">
        ...
        <?php
            $subtotalcart2= 0;
            $no=1;
            $data_produk1=mysqli_query($conn,"SELECT * FROM keranjanglog c, produklog p
    ...
    <table class="table table-striped table-sm table-bordered dt-responsive nowrap print-none" id="cart" width="100%">
                        ...
                        <tbody>
                            <?php
                                $subtotalcart1= 0;
                                $no=1;
                                $data_produk=mysqli_query($conn,"SELECT * FROM keranjanglog c, produklog p
                                WHERE p.idproduk=c.idproduk ORDER BY idcart ASC");
                        ...
                        if(isset($_POST['upone'])){
                            ...
                            $cekBarang2 = mysqli_query($conn, "SELECT * FROM keranjanglog WHERE idcart='$idcartt'");
                            ...
                            $insert1 = mysqli_query($conn, "UPDATE produklog SET stock='$sisa1' WHERE idproduk='$idproduk'");
                                    if($insert1){
                                        //update stok
                                        $hapus_data = mysqli_query($conn, "DELETE FROM keranjanglog WHERE idcart ='$idcartt'");
                                        echo '<script>alert("Data Berhasil Di Hapus");window.location="indexlog.php"</script>';
                                    } else {
...
if(isset($_POST['save'])){
    ...
    $updatetkeranjang = mysqli_query($conn,"UPDATE keranjanglog SET
      no_nota='$nonota'") 
     or die (mysqli_connect_error()); 

     $ambildatacart = mysqli_query($conn,"INSERT INTO tb_notalog
     (no_nota,idproduk,quantity)
    SELECT no_nota,idproduk,quantity FROM keranjanglog")
     or die (mysqli_connect_error()); 

     $insertlaporan = mysqli_query($conn, "INSERT INTO laporanlog (no_nota,id_printer,catatan,pembayaran,kembalian,totalbeli,nama_kasir)
      VALUES ('$nonota','$idpell','$catatan','$pembayaran','$kembalian','$totalbeli','$namakasir')");

    $hapusdatacart = mysqli_query($conn,"DELETE FROM keranjanglog");
