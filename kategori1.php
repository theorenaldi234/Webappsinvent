...
<?php
            if(isset($_POST['addkategori']))
            {
                ...
                $tambahkat = mysqli_query($conn,"INSERT INTO kategori1 (nama_kategori) values ('$namakategori')");
                if ($tambahkat){
                    echo '<script>alert("Berhasil Menambahkan Data");window.location="kategori1.php"</script>';
                } else {
    ...
    <div class="card-header">
        ...
        <?php 
            if(!empty($_GET['edit'])){
                ...
                $edit = mysqli_query($conn,"SELECT * FROM kategori1 WHERE idkategori='$idkategori'");
                while($e=mysqli_fetch_array($edit)){
                    ...
                    <div class="input-group">
                        ...
                        <div class="input-group-append">
                            ...
                            <a href="kategori1.php" class="btn btn-danger btn-xs py-1 px-2 ml-1"><i class="fas fa-times"></i>
                ...
                if(isset($_POST['update'])){
                    $namakategori = htmlspecialchars($_POST['nama_kategori']);
                    $editup = mysqli_query($conn,"UPDATE kategori1 SET nama_kategori='$namakategori' WHERE idkategori='$idkategori'");
                    echo '<script>alert("Berhasil Update Kategori");window.location="kategori1.php"</script>';
                    }
            } else { ?>
        ...
        <div class="card-body">
            <table class="table table-striped table-sm table-bordered dt-responsive nowrap" id="table" width="100%">
                        ...
                        <tbody>
                            <?php
                                $no=1;
                                $data_produk=mysqli_query($conn,"SELECT * FROM kategori1 ORDER BY idkategori ASC");
...
<?php 
    if(!empty($_GET['hapus'])){
        ...
        $hapus_data = mysqli_query($conn, "DELETE FROM kategori1 WHERE idkategori='$idkategori'");
        if($hapus_data){
            echo '<script>alert("Berhasi Hapus Kategori");window.location="kategori1.php"</script>';
        } else {
