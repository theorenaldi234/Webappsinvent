    ...
    if(isset($_POST['login'])){
        ...
        $cariuser = mysqli_fetch_assoc($queryuser);
            ...
            if( password_verify($pass, $cariuser['password']) ) {
                ...
                if($cariuser){ 
                    echo '<script>alert("Data yang anda masukan benar");window.location="printer.php"</script>';
                }else{
                    echo '<script>alert("Data yang anda masukan salah");history.go(-1);</script>';
                }
                echo '<script>alert("Anda Berhasil Login");window.location="printer.php"</script>';
            } else {
...
<div class="row justify-content-center">
    ...
    <h1 class="h3 text-center mb-3" style="font-weight:600;">IT<span class="text-primary">Inventory</span></h1>
            <div class="card-body bg-white shadow-sm">
                ...
                <div class="col-4 pl-1 mt-2">
                    <button class="btn btn-secondary btn-block" name="login" type="submit">
                       <i class="fa fa-user-circle"></i> Sign Up</button>
                </div>
                </div>
            </form>
        </div>
        <div class="text-center small mt-4">
            <p class="text-muted">IT © <?php echo date('Y');?> - By : <a href="" class="span text-primary">B</a></p>
        </div>
    </div>
