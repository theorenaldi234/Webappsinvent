...
<div class="col-lg-3 mb-3">
    <div class="card small mb-3">
        <div class="card-header p-2">
            <div class="card-tittle"><i class="far fa-file mr-1"></i> Out Logistik</div>
        </div>
    ...
    <div class="card small mb-3">
        <div class="card-header p-2">
            <div class="card-tittle"><i class="far fa-user mr-1"></i> Informasi User
        ...
        <div class="card-body p-2">
            ...
            if(isset($_POST['alamat_pelanggan']))
            {
                ...
                if ($tambahPel){
                    echo '<script>alert("Tambah Data Pelanggan Berhasil");window.location="indexcus.php"</script>';
                } else {
            ...
            ?>
                <form method="post">
                    <div class="row">
                        <div class="col-4 mb-2 text-right pt-1 pr-1 text-primary">Nama User : </div>
                        <div class="col-8 mb-2 pl-0">
                        ...
                        <div class="col-4 text-right pt-1 pr-1 text-primary">NIK : </div>
                        <div class="col-8 pl-0">
            ...
            <div id="Ada1">
            <div class="row">
                <div class="col-4 mb-2 text-right pt-1 pr-1">Nama User : </div>
                <div class="col-8 mb-2 pl-0">
                <?php 
                ...
                <div class="col-4 text-right pt-1 pr-1">NIK : </div>
                <div class="col-8 pl-0">
...
<div class="col-lg-9" id="print">
    ...
    <div class="row print-none">
        ...
        <?php 
            $barang=mysqli_query($conn, "SELECT * FROM produklog ORDER BY idproduk ASC");
...
<!-- kurang stok logistik -->
<?php
if(isset($_POST['tambahcuy'])){
    $idproduk  = $_POST['idproduk'];
    $quantity  = $_POST['quantity'];
    $cekBarang = mysqli_query($conn, "SELECT * FROM produklog WHERE idproduk='$idproduk'");
    $cekBarang2 = mysqli_query($conn, "SELECT * FROM produk WHERE idproduk='$idproduk'");
    $stocknya  = mysqli_fetch_array($cekBarang);
    $stocknya2  = mysqli_fetch_array($cekBarang2);
    $stock2    = $stocknya2['stock'];
    $stock     = $stocknya['stock'];
    $sisa      = $stock-$quantity;
    $hasilcus  = $stock2+$quantity;
    
 if ($stock < $quantity) {
    echo '<script>alert("Oops! Jumlah pengeluaran lebih besar dari stok ...");window.location="indexcus.php"</script>';
    }   
    else{
     $insert = mysqli_query($conn, "INSERT INTO keranjangcus (idproduk,quantity) VALUES ('$idproduk','$quantity')");
      if($insert){
        $upstok = mysqli_query($conn, "UPDATE produklog SET stock='$sisa' WHERE idproduk='$idproduk'");
        $upstok1 = mysqli_query($conn, "UPDATE produk SET stock='$hasilcus' WHERE idproduk='$idproduk'");
        echo '<script>window.location="indexcus.php"</script>';
     }
...
<div class="d-none pt-5 px-4 print-show">
    <div class="row">
        ...
        <?php
            $subtotalcart2= 0;
            $no=1;
            $data_produk1=mysqli_query($conn,"SELECT * FROM keranjangcus c, produklog p
    ...
    <table class="table table-striped table-sm table-bordered dt-responsive nowrap print-none" id="cart" width="100%">
                        ...
                        <tbody>
                            <?php
                                $subtotalcart1= 0;
                                $no=1;
                                $data_produk=mysqli_query($conn,"SELECT * FROM keranjangcus c, produklog p
                                WHERE p.idproduk=c.idproduk ORDER BY idcart ASC");
                        ...
                        if(isset($_POST['upone'])){
                            ...
                            $cekBarang1 = mysqli_query($conn, "SELECT * FROM produklog WHERE idproduk='$idproduk'");
                            $cekBarang2 = mysqli_query($conn, "SELECT * FROM produk WHERE idproduk='$idproduk'");
                            $cekBarang3 = mysqli_query($conn, "SELECT * FROM keranjangcus WHERE idcart='$idcartt'");
                            $stocknya1  = mysqli_fetch_array($cekBarang1);
                            $stocknya2  = mysqli_fetch_array($cekBarang2);
                            $stocknya3  = mysqli_fetch_array($cekBarang3);
                            $stockk     = $stocknya1['stock'];
                            $stockk2     = $stocknya2['stock'];
                            $jml1     = $stocknya3['quantity'];
                            //menghitung sisa stok
                            $sisa1    =$stockk+$jml1;
                            $sisa2    =$stockk2-$jml1;
                            

                           $insert2 = mysqli_query($conn, "UPDATE produk SET stock='$sisa2' WHERE idproduk='$idproduk'");
                           $insert1 = mysqli_query($conn, "UPDATE produklog SET stock='$sisa1' WHERE idproduk='$idproduk'");
                                    if($insert1){
                                        //update stok
                                        $hapus_data = mysqli_query($conn, "DELETE FROM keranjangcus WHERE idcart ='$idcartt'");
                                        echo '<script>alert("Data Berhasil Di Hapus");window.location="indexcus.php"</script>';
                                    } else {
...
if(isset($_POST['save'])){
    ...
    $updatetkeranjang = mysqli_query($conn,"UPDATE keranjangcus SET
      no_nota='$nonota'") 
     or die (mysqli_connect_error()); 

     $ambildatacart = mysqli_query($conn,"INSERT INTO tb_notacus
     (no_nota,idproduk,quantity)
    SELECT no_nota,idproduk,quantity FROM keranjangcus")
     or die (mysqli_connect_error()); 

     $insertlaporan = mysqli_query($conn, "INSERT INTO laporancus (no_nota,idpelanggan,catatan,pembayaran,kembalian,totalbeli,nama_kasir)
      VALUES ('$nonota','$idpell','$catatan','$pembayaran','$kembalian','$totalbeli','$namakasir')");

    $hapusdatacart = mysqli_query($conn,"DELETE FROM keranjangcus");
